...
(sf_four<- sf_nc_county %>%
    filter(county %in%
           c("guilford", "randolph", "davidson", "forsyth")))
...
(max(st_distance(sf_site_four_utm)))
...
(sf_site_four_utm <- st_transform(sf_site_four, 
                                  crs = 32617))

sf_site_four_buffer<- sf_site_four_utm %>% 
  st_buffer(dist = 3000)
...
(spr_crop_proj <- project(spr_crop_four,
                              y = "EPSG:32617",
                              method = "near"))
...
ggplot()+
  ...
  geom_sf(data = sf_site_four_buffer,
          alpha = 0.25)
...
(df_crop_frac <- exact_extract(x = spr_crop_proj,
                              y = sf_site_four_buffer,
                              fun = "mean",
                              append_cols = TRUE,
                              progress = FALSE))
df_crop_frac %>% 
  filter(mean == max(mean))
#Site ID : finsync_nrs_nc-10113 at 0.16
