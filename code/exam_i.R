...
mtcars_cyl6 %>% filter(gear%in%c(3,5))
# Assign to: `mtcars_g35`

mtcars_g35<-mtcars_cyl6 %>% filter(gear%in%c(3,5))
...
mtcars_mpg25<-df_mtcars %>% filter(mpg> 25)
...
mtcars_mpg_by_gear<- df_mtcars %>% group_by(gear) %>%  summarize(min(mpg),max(mpg))
...
g_scat<-iris %>% ggplot(aes(x=Sepal.Width, y=Sepal.Length))+geom_point()
...
g_hist<-iris %>% ggplot(aes(x=Sepal.Width))+geom_histogram(binwidth = 0.5)
...
g_bplot2<-PlantGrowth %>% ggplot(aes(x=weight,y=group,fill = group))+geom_boxplot()+geom_point()
